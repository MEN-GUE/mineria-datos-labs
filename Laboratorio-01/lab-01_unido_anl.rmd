...
Con el resumen se observan rangos, valores faltantes y distribución general de cada variable. En particular, variables como *budget* y *revenue* tienen muchos ceros y valores extremadamente altos, lo que sugiere asimetría fuerte y presencia de outliers. Esto es importante para decidir transformaciones (por ejemplo log) y filtros antes de comparar películas.
...
La tabla clasifica cada variable según su naturaleza (cualitativa vs cuantitativa) y cómo debe analizarse. Esto ayuda a escoger gráficos y medidas adecuadas: frecuencias para cualitativas y resúmenes/dispersiones para cuantitativas. También aclara que algunas columnas son listas o multi-etiquetas (como *genres* o *actors*), por lo que requieren limpieza previa.
...
Las pruebas de Shapiro-Wilk sobre una muestra indican p-values muy pequeños, por lo que no se asume normalidad en las variables numéricas. Los histogramas confirman distribuciones sesgadas (especialmente *budget*, *revenue* y *voteCount*). En variables cualitativas, las tablas de frecuencia muestran categorías dominantes y muchos NA en campos como *homePage*. Por eso conviene usar medidas robustas y, cuando aplique, transformaciones logarítmicas.

## 4 Preguntas de negocio



Resultados e interpretación breve.
...
El ranking muestra que unas pocas producciones concentran los presupuestos más altos. Esto refleja que el gasto en cine está muy concentrado en franquicias o grandes estudios. Estos casos extremos pueden influir mucho en promedios, así que para comparaciones conviene revisar medianas o usar escalas log.
...
Las películas con mayores ingresos son blockbusters muy conocidos, lo que sugiere un mercado con alta concentración en pocos títulos. Comparar ingresos junto con presupuesto ayuda a distinguir éxito comercial real (rentabilidad) frente a solo alta inversión. Más adelante, indicadores como ROI o ganancia (revenue - budget) dan una visión más clara.
...
La película con más votos suele ser un título muy visto y discutido, por lo que el conteo de votos funciona como un proxy de alcance. Sin embargo, muchos votos no implican necesariamente mejor calificación, solo mayor exposición. Por eso es útil analizar *voteAvg* y *voteCount* conjuntamente.
...
Sin filtro, la peor calificación puede estar dominada por casos con muy pocos votos. Al aplicar un umbral de votos se obtiene un resultado más estable y representativo de la opinión general. Esta práctica reduce el sesgo por películas poco vistas con calificaciones extremas.
...
El conteo por año revela en qué periodos hubo más producción registrada en el dataset. El año con mayor número de películas destaca claramente en el gráfico, lo que puede deberse a cobertura del dataset o a un aumento real de estrenos. Esto también sugiere que análisis por año deberían considerar tamaños de muestra muy diferentes.
...
En las películas más recientes se observan géneros principales variados, con presencia fuerte de Drama y algunos NA por falta de etiquetado. A nivel global, el género predominante se identifica por frecuencia y sirve para entender la composición del catálogo. En las películas más largas, aparecen con frecuencia documentales o eventos especiales, lo cual tiene sentido por su formato.
...
Al calcular ganancia como *revenue - budget*, se comparan géneros en términos de resultado económico, no solo ingresos brutos. Los géneros líderes en ganancia total suelen ser los más producidos o los que tienen grandes éxitos puntuales. La ganancia promedio complementa la lectura porque controla por cantidad de películas.
...
La correlación positiva indica que, en general, películas con más actores tienden a asociarse con mayores ingresos, aunque no implica causalidad. El gráfico en escala log ayuda a ver la relación sin que los outliers dominen la visualización. La tendencia por año muestra cambios fuertes en el promedio de actores, por lo que conviene revisar calidad de datos y años atípicos.
...
La proporción de mujeres en el elenco muestra una correlación muy baja con popularidad y prácticamente nula con ingresos. Esto sugiere que, con esta medición, no hay una relación directa fuerte entre balance de género y desempeño comercial. Aun así, podrían existir efectos por género de película, país o época, que requerirían segmentación para evaluarse mejor.
...
## 5.1 Extra 1: ¿Es posible agrupar las películas en categorías financieras basándose en presupuesto e ingresos?
...
Los clusters separan películas en grupos con niveles distintos de presupuesto e ingresos, lo que ayuda a identificar perfiles financieros típicos. Un grupo suele representar producciones grandes, otro medianas y otro pequeñas. Esto es útil para segmentar análisis y comparar desempeño dentro de categorías similares.

## 5.2 Extra 2: ¿Existen grupos de películas 'de culto' (alta calificación, baja popularidad) vs 'virales'?
...
El agrupamiento distingue perfiles como películas bien calificadas pero con baja popularidad (posible 'culto') y películas muy populares ('virales'). La separación sugiere que popularidad y calificación no siempre van juntas. Este tipo de segmentación ayuda a pensar estrategias: algunas buscan prestigio y otras alcance.

## 5.3 Extra 3: ¿Cuáles son las películas más rentables en términos relativos (ROI)?
...
El ROI identifica películas que multiplicaron su inversión de forma extraordinaria, incluso si no tienen los ingresos absolutos más altos. Estas suelen ser producciones de bajo presupuesto que lograron gran taquilla. El ranking es útil para estudiar patrones de éxito eficiente y comparar con blockbusters.

## 5.4 Extra 4: ¿Las coproducciones (varios países) generan más ingresos promedio?
...
La comparación muestra si las coproducciones tienen, en promedio, ingresos diferentes a las producciones de un solo país. Si las medias son similares, el efecto podría ser pequeño o depender de otros factores como el género o el mercado objetivo. También conviene revisar la distribución, no solo el promedio, por posibles outliers.

## 5.5 Extra 5: ¿Existe una duración (runtime) 'ideal' que maximice la calificación promedio?
...
Al agrupar por rangos de duración, se observa qué intervalo tiene la calificación promedio más alta. Esto sugiere una duración 'cómoda' para el público, mientras que duraciones muy cortas o muy largas pueden penalizar la evaluación. De todos modos, el género influye: documentales y eventos suelen ser más largos.

## 5.6 Extra 6: ¿Se pueden identificar grupos según presupuesto y calificación? (¿El dinero compra buenas críticas?)
...
Los clusters por presupuesto y calificación muestran que gastar más no garantiza mejores críticas. Se pueden ver grupos con alto presupuesto y calificación media, y otros con menor presupuesto pero mejores notas. Esto sugiere que la recepción depende también de guion, dirección, actuación y expectativas del público.
